<form method="POST" action="{{ route('service.update',$service) }}" enctype="multipart/form-data">
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-6">
            <select name="status" id="status" class="form-control">
                <option {{ $service->status == 1 ? 'selected' : '' }} value="1">تعرضها</option>
                <option {{ $service->status == 0 ? 'selected' : '' }} value="0">لا تعرضها</option>
            </select>
            @error('status')
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-6">
            ...
            @enderror
            @foreach($service->getMedia('sliderFiles') as $media)
